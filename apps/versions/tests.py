...
class TestDownloadsBase(amo.test_utils.ExtraSetup, test_utils.TestCase):
    ...
    def assert_served_locally(self, response, file_=None):
        ...
        eq_(response['Content-Type'], 'application/x-xpinstall')
        eq_(response['Content-Disposition'],
            'attachment; filename="%s"' % file_.filename)
        eq_(response['X-Sendfile'],
            '%s/%s/%s' % (settings.REPO_PATH, self.addon.id,
                          file_.filename))

    def assert_served_by_mirror(self, response):
        eq_(response.status_code, 302)
        eq_(response['Location'],
            '%s/%s/%s' % (settings.FILES_HOST, self.addon.id,
                          self.file.filename))
