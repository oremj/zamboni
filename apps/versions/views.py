...
def download_file(request, file_id, type=None):
    ...
    def path(base):
        return posixpath.join(*map(smart_str, [base, addon.id, file.filename]))
    ...
    if (not (settings.DEBUG or attachment)
        ...
        # Send it to the mirrors.
        return http.HttpResponseRedirect(path(settings.FILES_HOST))
    else:
        # Serve it locally.
        r = http.HttpResponse()
        r['Content-Type'] = 'application/x-xpinstall'
        r['Content-Disposition'] = 'attachment; filename="%s"' % file.filename
        r['X-Sendfile'] = path(settings.REPO_PATH)
        return r
